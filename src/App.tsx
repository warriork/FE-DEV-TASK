...
<main className='main'>
    ...
    <div className="products">
        {
        products.filter(product=>filter==='kategorie'? product : 
        product.category === filter).map(product=>(
        <div className="product" key={product.id}>
            <div className="product-img-wrapper">
            <img src={product.img} alt={product.name} />
            </div>
            <p className="product-avaliability">{product.isAvaliable?'Skladem': 'neni skladem'}</p>
            <p className="product-description">{product.description.length<25 ? product.description : product.description.slice(0, 25)+'...'}</p>
